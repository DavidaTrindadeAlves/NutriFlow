...
export function calculateComposition(input: AssessmentCalculationInput): AssessmentCalculationResult {
  ...
  if (protocol === 'Pollock 7 Dobras' || protocol === 'pollock_7') {
    ...
    if (gender === 'M') {
      ...
      if (skinfoldsSum > 0) {
        bodyDensity =
          1.09886 -
          0.000676 * skinfoldsSum +
          0.0000013 * Math.pow(skinfoldsSum, 2) -
          0.000438 * validAge;
        bodyFatPercentage = siriFormula(bodyDensity);
      }
    }
    ...
    if (gender === 'M') {
      ...
      if (skinfoldsSum > 0) {
        bodyDensity = 1.17136 - 0.06706 * Math.log10(skinfoldsSum);
        bodyFatPercentage = siriFormula(bodyDensity);
      }
    } else {
      skinfoldsSum = get('subscapular') + get('suprailiac') + get('thigh');
      if (skinfoldsSum > 0) {
        bodyDensity = 1.1665 - 0.07063 * Math.log10(skinfoldsSum);
        bodyFatPercentage = siriFormula(bodyDensity);
      }
    }
  } else {
    // Básico / Bioimpedância / Estimativa Deurenberg
    const genderVal = gender === 'M' ? 1 : 0;
    const estFat = 1.2 * bmi + 0.23 * validAge - 10.8 * genderVal - 5.4;
    bodyFatPercentage = Math.min(Math.max(Number(estFat.toFixed(2)), 5.0), 55.0);
  }
  ...
  return {
    bmi,
    bmiClassification,
    skinfoldsSum: Number(skinfoldsSum.toFixed(1)),
